 ...
 */
export function makePalette(original: string | HSV, spec = MATERIAL_2014, tolerance = { lightness: TOLERANCE_LIGHTNESS, chroma: TOLERANCE_CHROMA, closest: -1 }) {
  ...
  return palette.map((specColors, index) => {
    ...
    let chroma = isGrey ? specColors[3] - delta.c : specColors[3] - delta.c * Math.min(tolerance.chroma[index] / tolerance.chroma[tolerance.closest], 1.25)
